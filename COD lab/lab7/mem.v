module mem#(
    ...
    parameter LINE_OFFSET_WIDTH = 2,    // 行偏移位宽，决定一行的字数 2^2=4字
    parameter SPACE_OFFSET      = 2,    // 一个地址空间占1个字节，因此一个字需要4个地址空间，由于假设为整字读取，处理地址的时候可以默认后两位为0
    parameter MEM_ADDR_WIDTH    = 10,   // 模拟主存BRAM的大小，地址只有10位；为了简化，这里假设内存地址宽度为10位（CPU请求地址仍然是32位，只不过我们这里简化处理，截断了高位） 
    ...
    localparam LINE_WIDTH = 32 << LINE_OFFSET_WIDTH;    //一个字32位，计算出一行数据总位宽
    localparam cnt_max = 5; //更改这个值可以模拟不同的延迟，为了仿真方便可以设置成5

    wire [MEM_ADDR_WIDTH - 1:0] addr;   //BRAM内部地址线
    assign addr = mem_addr[MEM_ADDR_WIDTH+LINE_OFFSET_WIDTH + SPACE_OFFSET:LINE_OFFSET_WIDTH + SPACE_OFFSET];   //截取mem_addr的[14:4]位
    wire [LINE_WIDTH - 1:0] mem_data;   //一个cache line数据的大小
    ...
    mem_bram #(
        ...
        .raddr(addr),
        .waddr(addr),
        .din(mem_wdata),    //Data Input
        .we(mem_w && (cnt == cnt_max)), //Write Enable
        .dout(mem_data)
    );
