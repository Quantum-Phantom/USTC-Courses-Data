...
module cache_tb();
    ...
    parameter WAY_NUM           = 8;   // Cache N路组相联，仅要求支持 2 / 4 / 8 / 16
    ...
    // 性能评估计数器
    reg [31:0] hit_cnt      = 0;   // 命中次数
    reg [31:0] miss_cnt     = 0;   // 缺失次数
    reg [31:0] total_cycles = 0;   // 总时钟周期数
    reg        test_done     = 0;   // 测试完成标志
    reg        prev_miss     = 0;   // 上一周期的miss值，用于检测上升沿
    ...
    // 性能评估计数逻辑
    always @(posedge clk) begin
        if (stat && !test_done) begin
            total_cycles <= total_cycles + 1;
            // 检测miss上升沿，统计缺失次数
            if (miss && !prev_miss) begin
                miss_cnt <= miss_cnt + 1;
            end
            prev_miss <= miss;
        end
    ...
    always @(posedge clk) begin
        if (test_cnt == READ_NUM + WRITE_NUM) begin
            test_done <= 1;
            // 计算命中次数 = 总访问次数 - 缺失次数
            hit_cnt <= (READ_NUM + WRITE_NUM) - miss_cnt;
            if (diff == 0) begin
                $display("========== %d way TEST PASSED ==========",WAY_NUM);
            end else begin
                $display("========== %d way TEST FAILED ==========",WAY_NUM);
            end
            $display("========== PERFORMANCE REPORT ==========");
            $display("Total Accesses : %4d", READ_NUM + WRITE_NUM);
            $display("Hit Count      : %4d", (READ_NUM + WRITE_NUM) - miss_cnt);
            $display("Miss Count     : %4d", miss_cnt);
            $display("Hit Rate       : %4d / %4d = %d.%02d%%", 
                     (READ_NUM + WRITE_NUM) - miss_cnt,
                     READ_NUM + WRITE_NUM,
                     (((READ_NUM + WRITE_NUM) - miss_cnt) * 10000) / (READ_NUM + WRITE_NUM) / 100,
                     (((READ_NUM + WRITE_NUM) - miss_cnt) * 10000) / (READ_NUM + WRITE_NUM) % 100);
            $display("Total Cycles   : %d", total_cycles);
            $display("IPC (ideal)    : %d / %d = %d.%02d%%",
                     READ_NUM + WRITE_NUM,
                     total_cycles,
                     ((READ_NUM + WRITE_NUM) * 10000) / total_cycles / 100,
                     ((READ_NUM + WRITE_NUM) * 10000) / total_cycles % 100);
            $display("==========================================");
            $finish;
        end
