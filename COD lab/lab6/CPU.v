...
// =====================IF 阶段 (Instruction Fetch)
    ...
    PC u_PC (
        .clk(clk),
        .rst(rst),
        .en(global_en & ~stall_pc), //stall时PC不更新
...
// ====================EX 阶段 (Execute)
    ...
    wire [31:0] rf_rd0_fd;
    wire [31:0] rf_rd1_fd;
    wire [31:0] rf_src0_ex;
    wire [31:0] rf_src1_ex;
    wire        rf_rd0_fe;
    wire        rf_rd1_fe;
    wire [31:0] imm_ex;
    wire [4:0]  rf_wa_ex;
    ...
    // Segment control unit: generate stall/flush for pipeline registers
    SegCtrl u_SegCtrl (
        .rf_we_ex(rf_we_ex),
        ...
        .rf_ra0_id(rf_ra0_id),
        .rf_ra1_id(rf_ra1_id),
        .npc_sel_ex(npc_sel_ex),   //0: PC+4, 1: Branch/Jal Target, 2: Jalr Target
    ...
    wire [31:0] alu_src0_ex = alu_src0_sel_ex ? pc_ex : rf_src0_ex;
    wire [31:0] alu_src1_ex = alu_src1_sel_ex ? imm_ex : rf_src1_ex;
    ...
    BRANCH u_BRANCH (
        .br_type(br_type_ex),   // DECODER生成的分支/跳转类型
        .br_src0(rf_src0_ex),
        .br_src1(rf_src1_ex),

        .npc_sel(npc_sel_ex)    //0: PC+4, 1: Branch/Jal Target, 2: Jalr Target
    );
...
// ============================WB 阶段 (Write Back) & Commit
    ...
    Fowarding u_Fowarding (
        .rf_ra0_ex(rf_ra0_ex),  //EX寄存器读地址
        .rf_ra1_ex(rf_ra1_ex),

        .rf_wa_mem(rf_wa_mem),  //mem寄存器写地址
        .rf_wa_wb(rf_wa_wb),
        .rf_we_mem(rf_we_mem),  //mem寄存器写使能
        .rf_we_wb(rf_we_wb),
        .rf_wd_mem(alu_res_mem),    //mem阶段，ALU计算结果
        .rf_wd_wb(rf_wd_wb),

        .rf_rd0_fe(rf_rd0_fe),  //前递使能
        .rf_rd1_fe(rf_rd1_fe),
        .rf_rd0_fd(rf_rd0_fd),  //前递数据
        .rf_rd1_fd(rf_rd1_fd)
    );

    assign rf_src0_ex = rf_rd0_fe ? rf_rd0_fd : rf_rd0_raw_ex;  //ALU操作数来源
    assign rf_src1_ex = rf_rd1_fe ? rf_rd1_fd : rf_rd1_raw_ex;
